...
nsl_cmd_exec_get::nsl_cmd_exec_get() : nsl_cmd_exec("get")
{
	macro_total = 0;
...
nsl_cmd_exec_get::~nsl_cmd_exec_get()
{
}
int nsl_cmd_exec_get::exec(nsl_interpreter& nsl)
{
	nsl_string str;
	const char* mstr;
	const char* nstr;
...
//			NSL_SYSTEM->setApplicationModel();
	...
	cmd_error("cmd_get: Bad Attribute");

	return fg;
}
int nsl_cmd_exec_get::add_macro(nsl_cmd_macro* obj)
{
...
int nsl_cmd_exec_get::help(const char*)
{
	return 2;
}
