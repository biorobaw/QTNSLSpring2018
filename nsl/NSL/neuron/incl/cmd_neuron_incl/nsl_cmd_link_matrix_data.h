//
// 	nsl_cmd_link_matrix_data.h
//

#ifndef _NSL_CMD_LINK_MATRIX_DATA_H
#define _NSL_CMD_LINK_MATRIX_DATA_H
#include "nsl_cmd_link_data.h"
class nsl_cmd_link_matrix_data : public nsl_cmd_link_data
{
public:
		nsl_cmd_link_matrix_data();
		~nsl_cmd_link_matrix_data();

	int init_cmd_link_matrix_data();
	int set_cmd_link_matrix_data();
	...
	virtual int cmd_step(nsl_interpreter&);
		...
		int print_cmd_link_matrix_data(std::ostream&) const;
