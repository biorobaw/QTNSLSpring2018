...
#ifndef _NSL_CMD_LINK_DATA_H
#define _NSL_CMD_LINK_DATA_H
#include "nsl_cmd_link_layer.h"
class nsl_cmd_link_data : public nsl_cmd_link_layer
{
public:
		nsl_cmd_link_data();
		~nsl_cmd_link_data();

	int init_cmd_link_data();
	int set_cmd_link_data();
	...
	virtual int cmd_step(nsl_interpreter&);
		...
		int print_cmd_link_data(std::ostream&) const;
