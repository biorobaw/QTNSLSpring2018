...
// 	nsl_cmd_link_layer.h
//
#ifndef _NSL_CMD_LINK_LAYER_H
#define _NSL_CMD_LINK_LAYER_H
#include "nsl_cmd_layer.h"
class nsl_cmd_link_layer : public nsl_cmd_layer
{
public:
		nsl_cmd_link_layer();
		~nsl_cmd_link_layer();

	int init_cmd_link_layer();
	int set_cmd_link_layer();
	...
	virtual int cmd_step(nsl_interpreter&);
		...
		int print_cmd_link_layer(std::ostream&) const;
