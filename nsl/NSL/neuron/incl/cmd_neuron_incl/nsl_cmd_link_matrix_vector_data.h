//
// 	nsl_cmd_link_matrix_vector_data.h
//

#ifndef _NSL_CMD_LINK_MATRIX_VECTOR_DATA_H
#define _NSL_CMD_LINK_MATRIX_VECTOR_DATA_H
#include "nsl_cmd_link_matrix_data.h"
class nsl_cmd_link_matrix_vector_data : public nsl_cmd_link_matrix_data
{
public:
		nsl_cmd_link_matrix_vector_data();
		~nsl_cmd_link_matrix_vector_data();

	int init_cmd_link_matrix_vector_data();
	int set_cmd_link_matrix_vector_data();
	...
	virtual int cmd_step(nsl_interpreter&);
		...
		int print_cmd_link_matrix_vector_data(std::ostream&) const;
