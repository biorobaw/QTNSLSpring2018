//
// 	nsl_cmd_link_vector_matrix.h
//

#ifndef _NSL_CMD_LINK_VECTOR_MATRIX_H
#define _NSL_CMD_LINK_VECTOR_MATRIX_H
#include "nsl_cmd_link_matrix.h"


class nsl_cmd_link_vector_matrix : public nsl_cmd_link_matrix
{
public:
		nsl_cmd_link_vector_matrix();
		~nsl_cmd_link_vector_matrix();

	int init_cmd_link_vector_matrix();
	int set_cmd_link_vector_matrix();
	...
	virtual int cmd_step(nsl_interpreter&);
		...
		int print_cmd_link_vector_matrix(std::ostream&) const;
