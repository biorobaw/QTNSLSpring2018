//
// 	nsl_cmd_link_matrix_matrix.h
//

#ifndef _NSL_CMD_LINK_MATRIX_MATRIX_H
#define _NSL_CMD_LINK_MATRIX_MATRIX_H
#include "nsl_cmd_link_matrix.h"
class nsl_cmd_link_matrix_matrix : public nsl_cmd_link_matrix
{
public:
		nsl_cmd_link_matrix_matrix();
		~nsl_cmd_link_matrix_matrix();

	int init_cmd_link_matrix_matrix();
	int set_cmd_link_matrix_matrix();
	...
	virtual int cmd_step(nsl_interpreter&);
		...
		int print_cmd_link_matrix_matrix(std::ostream&) const;
