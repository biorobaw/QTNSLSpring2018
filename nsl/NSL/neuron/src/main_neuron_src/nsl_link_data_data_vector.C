...
nsl_link_data_data_vector::nsl_link_data_data_vector(nsl_layer& f,
	nsl_layer& t,nsl_layer& w,int s) : nsl_link_data_vector(f,t,w,s)
{
}

nsl_link_data_data_vector::~nsl_link_data_data_vector()
{
}

int nsl_link_data_data_vector::run()
{
	nsl_layer* out;

	if (link_layer == NULL)
	{
		cmd_out("nsl_link_data_data_vector::run: NULL link_layer");
		return 1;
	}
...
//		out->print(cout);
	...
	{
		cmd_out("nsl_link_data_data_vector::run: NULL from/output");
		return 1;
	}
