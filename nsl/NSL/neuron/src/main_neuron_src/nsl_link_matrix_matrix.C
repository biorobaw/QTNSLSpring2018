...
nsl_link_matrix_matrix::nsl_link_matrix_matrix(nsl_layer& f,
...
nsl_link_matrix_matrix::~nsl_link_matrix_matrix()
{
}

int nsl_link_matrix_matrix::run()
{
	nsl_layer* out;

	if (link_layer == NULL)
	{
		cmd_out("nsl_link_matrix_matrix::run: NULL link_layer");
		return 1;
	}
...
//		out->print(cout);
	...
	{
		cmd_out("nsl_link_matrix_matrix::run: NULL from/output");
		return 1;
	}
