...
nsl_cmd_icon_stim_data::nsl_cmd_icon_stim_data()
{
	set_type("input_data");
	set_name("icon_stim");
}
...
int nsl_cmd_icon_stim_data::cmd_clear(nsl_interpreter&)
{
 	return 1;
}
int nsl_cmd_icon_stim_data::cmd_create(nsl_interpreter& nsl)
{
	nsl_stim* stim;
  	nsl_layer* layer;
...
int nsl_cmd_icon_stim_data::cmd_reset(nsl_interpreter&)
{
 	return 1;
}
int nsl_cmd_icon_stim_data::cmd_update(nsl_interpreter&)
{
 	update_data(OBJ);

	return 1;
}
int nsl_cmd_icon_stim_data::init_data(nsl_base* stim)
{
	nsl_num_0* dat;
 	nsl_list* list = 0;
 	nsl_cmd_type* data;
 	...
 	if (list != NULL && stim != NULL)
	{
	    ...
	    {
		dat = ((nsl_icon_stim_data*) stim)->get_icon();
	    }
 	    else
		cmd_error("cmd_icon_stim_data: NULL 'name' obj");
	}
	else
	    cmd_error("NULL OBJ");
...
int nsl_cmd_icon_stim_data::update_data(nsl_base* stim,
	int,int,int,int,int,int,int,int,int,int)
{
	...
	if (list != NULL && stim != NULL)
	{
	    if ((data = (nsl_cmd_type*) list->get_by_name("icon")) != NULL)
	    {
//		dat = (nsl_num_data*) data->get_lobj();
//		*((nsl_icon_stim_data*) stim)->get_icon() = *dat;
	    }
 	    else
		cmd_error("cmd_icon_stim_data: NULL 'name' obj");
	}
