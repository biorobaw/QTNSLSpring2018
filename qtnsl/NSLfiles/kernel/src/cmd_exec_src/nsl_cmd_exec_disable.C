/* SCCS  @(#)nsl_cmd_exec_disable.C	1.1---95/10/13--16:35:10 */
/**************************************************************/
/*                                                            */
/*                   nsl_cmd_exec_disable.C                   */
/*                                                            */
/**************************************************************/
...
nsl_cmd_exec_disable::nsl_cmd_exec_disable() : nsl_cmd_exec("disable")
{
}

nsl_cmd_exec_disable::~nsl_cmd_exec_disable()
{
}
int nsl_cmd_exec_disable::exec(nsl_interpreter& nsl)
{
	nsl_string str;
	int fg = 2;
	nsl_cmd_type* obj;
	...
	nsl >> str; // disable: layer,etc

	if ((obj = (nsl_cmd_type*) list->get_mi_by_type(str)) != 0)
	{
		fg = obj->cmd_disable(nsl);
		return fg;
	}
	else
	{
		...
		nsl.putback(str,1);
		nsl.putback("disable module",2);
		fg = ((nsl_cmd_buf*) m_parent)->run(&nsl);
 	}

	return fg;
}
int nsl_cmd_exec_disable::help(const char* str)
{
