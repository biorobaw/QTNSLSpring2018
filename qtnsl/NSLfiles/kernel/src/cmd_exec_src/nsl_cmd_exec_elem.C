/* SCCS  @(#)nsl_cmd_exec_elem.C	1.1---95/10/13--16:35:10 */
/**************************************************************/
/*                                                            */
/*                   nsl_cmd_exec_elem.C                      */
/*                                                            */
...
nsl_cmd_exec_elem::nsl_cmd_exec_elem() : nsl_cmd_exec("elem")
{
// set elem_data_value
}

nsl_cmd_exec_elem::~nsl_cmd_exec_elem()
{
}
int nsl_cmd_exec_elem::exec(nsl_interpreter& nsl)
{
	nsl_string str;
	int fg = 2;
	nsl_cmd_type* obj;
	...
	else
	{
		...
		nsl.putback(str,1);
		nsl.putback("elem layer",2);
		fg = ((nsl_cmd_buf*) m_parent)->run(&nsl);
 	}

	return fg;
}
int nsl_cmd_exec_elem::help(const char* str)
{
