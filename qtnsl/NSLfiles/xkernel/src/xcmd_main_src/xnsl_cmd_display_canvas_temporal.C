...
int xnsl_cmd_display_canvas_temporal::update_data(nsl_base* win,  //this method executes in regards to the actual command instantiating a temporal graph or changing it
	int x,int y,int w,int h,int wx0,int wy0,int wx1,int wy1,int wxs,int wys)//it does not involve changes from the simulation of a model -bn7
{
	xnsl_graph* cw;
 	nsl_list* list = 0;
	nsl_list* dlist = 0;
