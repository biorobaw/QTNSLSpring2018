...
/**************************************************************/
/*                                                            */
/*           xnsl_cmd_display_canvas_area_level_vector.C      */
/*                                                            */
/**************************************************************/
...
xnsl_cmd_display_canvas_area_level_vector::
	xnsl_cmd_display_canvas_area_level_vector()
{
        set_type("display_canvas_area_level_graph_vector");
}
xnsl_cmd_display_canvas_area_level_vector::
	~xnsl_cmd_display_canvas_area_level_vector()
{
}
int xnsl_cmd_display_canvas_area_level_vector::create_data(nsl_base* win,
...
int xnsl_cmd_display_canvas_area_level_vector::update_data(nsl_base* win,
	int x,int y,int w,int h,
	int wx0,int wy0,int wx1,int wy1,int wxs,int wys)
{
...
int xnsl_cmd_display_canvas_area_level_vector::create_graph(nsl_base* win,
	int x,int y,int w,int h,
	...
	const char*)
{
	...
	{
            cw = new xnsl_area_level_graph_vector((xnsl_display_canvas*) win,layer);
	    ...
	    strcpy(str,win->get_name());
  	    ...
  	    if (cw != NULL && (canvas = ((xnsl_window*) win)->
						get_widget()) != NULL)
	    {
		cw->set_name("graph"); // data name in data list
		cw->set_wname(str);
   		cw->create_widget(str,canvas,x,y,w,h,wx0,wy0,wx1,wy1,wxs,wys);
	    }
  	}
	else
	    cmd_error("cmd_display_canvas_area_level_vec: Cannot create graph");

 	return 1;
}
int xnsl_cmd_display_canvas_area_level_vector::update_graph(nsl_base* win,
	int x,int y,int w,int h,
	int wx0,int wy0,int wx1,int wy1,int wxs,int wys,nsl_layer*,const char*)
{
	    ...
	    (cw = (xnsl_graph*) list->get_by_name("graph")) != NULL)
	{
		    ...
		    ((xnsl_area_level_graph_vector*) cw)->set_i0(i);
  	    	}
            	else
	    	...
	    	{
	            i = ((nsl_int_data*) data)->get_value();
		    ((xnsl_area_level_graph_vector*) cw)->set_i1(i);
  	    	}
            	else
		...
		cmd_error("cmd_display_canvas_area_level_vector: NULL window");
